...
CplusplusErrorHandler CplusplusParserTD::error_handler;

CplusplusParserTD::CplusplusParserTD(Scanner *scanner) : Parser(scanner)
{
    CplusplusError::initialize();
}

CplusplusParserTD::CplusplusParserTD(CplusplusParserTD *parent)
    : Parser(parent->get_scanner())
{
}

void CplusplusParserTD::parse() throw (string)
{
    Token *token = nullptr;
    ...
    while ((token = next_token(token)) != nullptr)
    {
        ...
        switch ((CplusplusTokenType) token_type)
        {
            case PT_STRING:
            {
                type_str = "STRING";
            ...
            default:  // reserved word or special character
            {
                ...
                {
                    type_str =
                        CplusplusToken::SPECIAL_SYMBOL_NAMES[
                                           (CplusplusTokenType) token_type];
                }
        ...
        {
            CplusplusErrorCode error_code =
                                    (CplusplusErrorCode) cast(value, int);
...
int CplusplusParserTD::get_error_count() const
{
